...
import { TimeSegment, TimelineEvent } from '../../types/timeLineType';
...
import { historicalDates as ru } from "../../assets/locales/ru.json"
...
const HistoricalDates = () => {
  ...
  const animateSegmentChange = useCallback(
    (newIndex: number) => {
      const currentRotation = timelineRef.current ? timelineRef.current.rotation() : 0;
  ...
  const handleDotClick = (index: number) => {
    if (index === activeIndex) return; // Не переключать, если уже активен
    animateSegmentChange(index);
  };

  // Обработчик клика по стрелкам слайдера (для Swiper)
  const handleSwiperPrev = () => swiperRef.current?.swiper?.slidePrev();
  const handleSwiperNext = () => swiperRef.current?.swiper?.slideNext();

  // Обработчик переключения сегмента (стрелки под годами)
  const handlePrevSegment = () => {
    const newIndex = (activeIndex - 1 + SEGMENTS_COUNT) % SEGMENTS_COUNT;
    animateSegmentChange(newIndex);
  };

  const handleNextSegment = () => {
    const newIndex = (activeIndex + 1) % SEGMENTS_COUNT;
    animateSegmentChange(newIndex);
  };
    ...
    <div className={styles.container} ref={containerRef}>
      ...
      <div className={styles["timeline__segment-info"]}>
        <span className={styles["timeline__segment-index"]}>
          {activeSegment.id.toString().padStart(2, '0')}/
          {SEGMENTS_COUNT.toString().padStart(2, '0')}
        </span>
      ...
      <div className={styles["timeline__visual-area"]}>
        <div className={styles["timeline__circle-container"]}>
          ...
          <div className={styles["timeline__years"]} ref={yearsRef}>
            <div className={styles["timeline__year timeline__year--start"]}>
              {activeSegment.startYear}
            </div>
            <div className={styles["timeline__year timeline__year--end"]}>
      ...
      <div className={styles["timeline__swiper-wrapper"]}>
        ...
        <div className={styles["timeline__swiper-nav"]}>
          <button className={styles["timeline__swiper-prev"]} onClick={handleSwiperPrev} aria-label="Предыдущее событие">&lt;</button>
          <button className={styles["timeline__swiper-next"]} onClick={handleSwiperNext} aria-label="Следующее событие">&gt;</button>
        </div>
      </div>
    </div>
  );
